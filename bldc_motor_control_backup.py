...
class BLDCMotorController:
    ...
    def connect(self):
        ...
        try:
            # Определяем тип интерфейса в зависимости от платформы и канала
            bustype = None
            
            if self.channel == 'vcan0' or self.channel == 'virtual':
                bustype = 'virtual'
            elif self.channel.startswith('can'):
                # Попытка использовать socketcan (Linux) или fallback на virtual
                try:
                    bustype = 'socketcan'
                except:
                    bustype = 'virtual'
            else:
                # Для Windows пробуем доступные интерфейсы
                # python-can поддерживает: ixxat, vector, nican, usb2can и др.
                bustype = 'virtual'  # По умолчанию используем виртуальный для тестов
