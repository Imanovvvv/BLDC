...
Используется библиотека pylkmotor для работы с протоколом двигателя.
Ммониторинг токов Id, Iq.
"""
...
try:
    ...
    print("Библиотека pylkmotor не найдена. Запуск в режиме симуляции.")
...
try:
    ...
    print("Библиотека matplotlib не найдена.")
    sys.exit(1)
...
class BLDCController:
    ...
    def __init__(self, use_simulator=False, channel=None, interface=None):
        ...
        if not use_simulator and PYLKMOTOR_AVAILABLE:
            try:
                ...
                self.connected = True
                print("Успешное подключение к двигателю.")
    ...
    def get_currents(self):
        ...
        try:
            ...
            print("Метод получения данных в pylkmotor не найден или изменился. Возврат симуляции.")
            return self.simulator.get_data()
